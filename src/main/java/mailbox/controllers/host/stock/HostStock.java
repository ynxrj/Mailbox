...
public class HostStock {
    ...
    public void handleAddBtn() throws IOException{
        if(!senderText.getText().equals("") && receiverText.getValue() != null && sizeText.getValue() != null) {
            if(senderText.getText().contains(",") || sizeText.getValue().equals(",")){
                ...
                if (confirmation.get() == ButtonType.OK) {
                    try {
                        ...
                        Letter letter = new Letter(senderText.getText(), receiverText.getValue(), sizeText.getValue(), path, currentHost.getName(), time, "InStock", "-");

                        stockList.addStock(letter);
                        stockData = new StockFileData("data", "stock.csv");
