...
public class HostPackage {
    ...
    @FXML
    private JFXTextField senderText, transText, trackText;
    @FXML
    private JFXComboBox<String> sizeText, receiverText;
    ...
    public void handleAddBtn() throws IOException{
        if(!senderText.getText().equals("") && receiverText.getValue() != null && !transText.getText().isEmpty() && !trackText.getText().isEmpty() && sizeText.getValue() != null) {
            if(senderText.getText().contains(",") || trackText.getText().contains(",") || transText.getText().contains(",") || sizeText.getValue().contains(",")){
                ...
                if (confirmation.get() == ButtonType.OK) {
                    try {
                        ...
                        Packages box = new Packages(senderText.getText(), receiverText.getValue(), sizeText.getValue(), transText.getText(), trackText.getText(), path, currentHost.getName(), time, "InStock", "-");

                        stockList.addStock(box);
                        stockData = new StockFileData("data", "stock.csv");
